<template>
   <div
ref="dropdown" class="dropdown-2" :class="{
      ...
      <div class="dropdown-2__input" :class="{ 'dropdown__input--disabled': disabled }">
         <div class="input-wrapper">
            <input
v-model="searchQuery" class="input" type="text" placeholder="Начните вводить название города"
               :disabled="disabled" @focus="activateDropdown" @keydown.enter.prevent="handleEnterKey">
         </div>
      </div>
      <ul v-if="isActive && cities.length" class="dropdown-2__list">
...
const handleEnterKey = () => {
   if (!isActive.value || !cities.value.length) return
